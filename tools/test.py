...
def inference_bfj(config, network, model_file, device, dataset, start, end, result_queue, result_queue_match):
    ...
    for (image, gt_boxes, im_info, ID, image_id) in data_iter:
        ...
        if config.test_nms_method == 'set_nms':
            ...
            assert pred_boxes.shape[-1] % 16 == 0, "Prediction dim Error!"
            pred_boxes = pred_boxes.reshape(-1, 16)
            pred_emb = pred_emb.reshape(-1, 32)
            keep = pred_boxes[:, 14] > config.pred_cls_threshold
            pred_boxes = pred_boxes[keep]
            pred_emb = pred_emb[keep]
            result = []
            result_emb = []
            for classid in range(class_num):
                keep = pred_boxes[:, 15] == (classid + 1)
                class_boxes = pred_boxes[keep]
                class_emb = pred_emb[keep]
                # print('class_boxes:', class_boxes.shape)
                keep = nms_utils.cpu_nms(class_boxes, config.test_nms)
                class_boxes = class_boxes[keep]
...
def match_body_face_bfj(pred_boxes, pred_emb, image_id):
    keep_body = pred_boxes[:, 15] == 1
    keep_face = pred_boxes[:, 15] == 2
    ...
    base_body_scores = body_boxes[:, 14]
    base_body_hooks = np.concatenate((body_boxes[:, 4:14:2].mean(1, keepdims=True), body_boxes[:, 5:15:2].mean(1, keepdims=True)), axis=1)

    base_face_boxes = face_boxes[:, :4]
    base_face_scores = face_boxes[:, 14]
    base_face_hooks = np.concatenate((face_boxes[:, 4:14:2].mean(1, keepdims=True), face_boxes[:, 5:15:2].mean(1, keepdims=True)), axis=1)
    ...
    if boxes.shape[-1] >= 8: # v2 or v3
        if embs is not None:
            result = [{'box':[round(i, 1) for i in box[:4].tolist()],
                   'score':round(float(box[14]), 5),
                   'tag':int(box[15]),
                   'emb':emb.tolist()} for box, emb in zip(boxes, embs)]
        else:
            result = [{'box':[round(i, 1) for i in box[:4].tolist()],
                    'score':round(float(box[14]), 5),
                    'tag':int(box[15])} for box in boxes]
