...
class RCNN(nn.Module):
    ...
    def forward(self, fpn_fms, rcnn_rois, rcnn_lmks=None, labels=None, tags=None, ious=None, centers=None, bbox_targets_dis=None):
        ...
        if self.training:
            ...
            pred_regression = pred_delta#torch.cat((pred_delta, pred_pos), dim=-1)            
            localization_loss = smooth_l1_loss(
                pred_regression,
                bbox_targets_dis[fg_masks, :4],
            ...
            if not skip_lmk:
                ...
                pred_delta1 = pred_delta1[lmk_masks, labels[lmk_masks], :]
                ...
                pred_pos_lmk_x = pred_pos_lmk[:, 4:14:2].mean(-1)
                pred_pos_lmk_y = pred_pos_lmk[:, 5:15:2].mean(-1)
                pred_cen_lmk = torch.cat((pred_pos_lmk_x.unsqueeze(-1), pred_pos_lmk_y.unsqueeze(-1)), dim=1)                
                gt_offset_lmk = bbox_targets_dis[lmk_masks]
                gt_lmk = restore_bbox(base_rois_lmks, gt_offset_lmk, True, True)
                lmk_target_x = gt_lmk[:, 4:14:2].mean(-1)
                lmk_target_y = gt_lmk[:, 5:15:2].mean(-1)
                ...
                loss_dict['loss_lmk'] = loss_rcnn_lmk + loss_lmk_cls + angular_loss_lmk
            ...
            pred_offset_lmk = pred_lmk1[:,10:].reshape(-1, 10)
            pred_offset = torch.cat((pred_delta, pred_offset_lmk), dim=1)
            pred_bbox = restore_bbox(base_rois, pred_offset, True, True)
            pred_bbox = torch.cat([pred_bbox, pred_scores, tag], axis=1)    #x16
...
def restore_bbox(rois, deltas, unnormalize=True, lmks=False):
    if unnormalize:        
        if not lmks:
            ...
            std_opr = torch.tensor(config.bbox_14_normalize_stds[None, :]).type_as(deltas)
            mean_opr = torch.tensor(config.bbox_14_normalize_means[None, :]).type_as(deltas)
