...
class CrowdHuman(torch.utils.data.Dataset):
    def __init__(self, config, if_train):
        if if_train:
            ...
            config.image_folder = '../../data/CrownHuman/CrowdHuman_train/Images'
        else:
            self.training = False
            ...
            config.image_folder = '../../data/CrownHuman/CrowdHuman_val/Images'
...
def flip_boxes(boxes, im_w):
    ...
    for i in range(flip_boxes.shape[0]):
        ...
        if flip_boxes.shape[1] > 10:            
            flip_boxes[i, 6:10:2] = im_w - boxes[i, 8:4:-2] - 1
            flip_boxes[i, 12:16:2] = im_w - boxes[i, 14:10:-2] - 1
            flip_boxes[i, 10] = im_w - boxes[i, 10] - 1
    return flip_boxes
