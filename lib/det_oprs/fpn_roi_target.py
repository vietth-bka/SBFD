...
def fpn_roi_target_bfj(rpn_rois, im_info, gt_boxes, top_k=1):
    ...
    for bid in range(config.train_batch_per_gpu):
        ...
        # tags
        tags = gt_boxes_perimg[gt_assignment, 5]
        ...
        # lmk rois
        lmk_rois = gt_boxes_perimg[gt_assignment, 6:]
        ...
        if config.rcnn_bbox_normalize_targets:
            std_opr = torch.tensor(config.bbox_14_normalize_stds[None, :]).type_as(bbox_targets)
            mean_opr = torch.tensor(config.bbox_14_normalize_means[None, :]).type_as(bbox_targets)
            minus_opr = mean_opr / std_opr
            bbox_targets = bbox_targets / std_opr - minus_opr
        bbox_targets = bbox_targets.reshape(-1, top_k * 14)
        return_rois.append(rois)
